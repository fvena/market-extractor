...
/** Required fields for BME Alternatives details - missing these triggers error status */
export const BME_ALTERNATIVES_REQUIRED_FIELDS = [
  "name",
  "sector",
  "lastPrice",
  "marketCap",
...
export const BME_ALTERNATIVES_OPTIONAL_FIELDS = [
  "address",
  "auditor",
  "contact",
  "liquidityProvider",
...
export interface BmeAlternativesDetails {
  // === From valor section (#ss_valor) ===
  address?: string; // Address
  auditor?: string;

  contact?: string;

  // === Documents (sections #ss_documentacion and #ss_avisos) ===
  documents: BmeDocument[];
  // === Metadata ===
  errors?: string[]; // Errors during fetching
  fetchedAt: string; // ISO timestamp
  isin: string;
  // === From header (#Contenido_fCab) ===
  lastPrice: number;
  liquidityProvider?: string;
  marketCap: number; // Calculated: sharesOutstanding * lastPrice
  ...
  sharesOutstanding: number; // Outstanding shares

  status: "error" | "success" | "warning";

  suspendedDate?: string; // ISO format, undefined if not suspended

  ticker: string;

  tradingType?: string; // Trading type (Fixing, etc.)
  url: string; // Product page URL
  website?: string;
  // === Yearly history (table in page #Contenido_tblCapital) ===
  yearlyHistory: BmeYearlyData[];
}

/** Progress callback for detail fetching with phases */
export type DetailProgressCallback = (phase: string, detail?: string) => void;
...
/**
 * Complete BME Continuo product details
 */
export interface BmeContinuoDetails {
  // From ShareDetailsInfo
  address?: string;
  admittedCapital: number;
  companyKey: string;
