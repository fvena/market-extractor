...
import type { EuronextListing } from "../../types/types";
 ...
 * Column 4: Market
 */
export async function parseEuronextTable(page: Page, baseUrl: string): Promise<EuronextListing[]> {
  const items: EuronextListing[] = [];
  ...
  for (let index = 0; index < rowCount; index++) {
    ...
    const marketText = await getTextContent(marketCell);
    const markets = marketText.split(",").map((market) => market.trim());

    if (href && name) {
      const url = href.startsWith("http") ? href : baseUrl + href;
      items.push({ isin, markets, name, ticker, url });
    }
  }
