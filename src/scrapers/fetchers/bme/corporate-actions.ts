...
import type { BmeCorporateActions } from "../../../markets/bme/types";
import type { CorporateActionType } from "../../parsers/bme/corporate-actions";
import colors from "yoctocolors";
import { buildFormData, CookieJar, extractAspxTokens, fetchHtml } from "../../clients/http";
import { delay } from "../../clients/browser";
import { ACTION_TYPE_CONFIG, parseCorporateActionsPage } from "../../parsers/bme/corporate-actions";
import { formatSpanishDate } from "../../../helpers/parsing";
import { createSpinner, succeedSpinner, updateSpinner } from "../../../cli/utils/progress";
...
export async function fetchBmeAlternativesCorporateActions(
  ...
  try {
    ...
    for (const [index, actionType] of CORPORATE_ACTION_TYPES.entries()) {
      ...
      const spinner = createSpinner(`Fetching ${config.key}`);
      ...
      do {
        ...
        updateSpinner(
          spinner,
          `Fetching ${config.key} ${colors.dim(`- Page ${String(pageNumber)}`)}`,
        );
      ...
      succeedSpinner(spinner, `Fetched ${String(result[config.key].length)} ${config.key}`);
    }
