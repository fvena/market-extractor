...
/**
 * Parse detailed quote from getDetailedQuote endpoint
 * Extracts: Last traded price, Valuation Close, market name
 */
export function parseDetailedQuote(html: string): EuronextQuote {
  const result: EuronextQuote = {};
  ...
  // Market name is the text of the first node inside .address__text before <br>
  const addressText = document_.querySelector(".address__text");
  if (addressText) {
    const htmlContent = addressText.innerHTML;
    const beforeBr = htmlContent.split(/<br/i)[0];
    if (beforeBr) {
      result.marketName = cleanText(beforeBr.replaceAll(/<[^>]*>/g, ""));
    }
  }

  // Last traded price
  const lastTradedPrice = document_.querySelector("#header-instrument-price");
  if (lastTradedPrice) {
    result.lastTradedPrice = parseEuronextNumber(cleanText(lastTradedPrice.textContent));
  }

  // Valuation close
  const valuationClose = document_.querySelector("#col-header-instrument-price");
  if (valuationClose) {
    result.valuationClose = parseEuronextNumber(cleanText(valuationClose.textContent));
  }
...
export function parsePriceHistoryTable(html: string): EuronextDailyPrice[] {
  ...
  for (const row of rows) {
    ...
    // 0: Date, 5: Close Price, 6: Number of shares (volume)
    ...
    if (date && closePrice !== undefined) {
      ...
      prices.push({
        closePrice,
        date,
        turnover: closePrice * actualVolume,
        volume: actualVolume,
      });
    }
  }
