...
import type { BmeDocument, BmeYearlyData } from "../../../markets/bme/types";
...
function parseYearlyHistoryTable(document: Document, errors: string[]): BmeYearlyData[] {
  ...
  for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
    ...
    const rowLabel = cleanText(cells[0]?.textContent ?? "").toLowerCase();
    ...
    for (let colIndex = 1; colIndex < cells.length && colIndex - 1 < years.length; colIndex++) {
      ...
      // Apply multiplier for values in thousands
      const isThousands =
        rowLabel.includes("miles") || rowLabel.includes("x 1.000") || rowLabel.includes("x1000");
      ...
      switch (metricKey) {
        ...
        case "marketCap": {
          yearData.marketCap = value * multiplier;
          break;
        }
        case "shares": {
          yearData.shares = value * multiplier;
          break;
        }
        case "turnover": {
          yearData.turnover = value * multiplier;
          break;
        }
        case "volume": {
          yearData.volume = value * multiplier;
          break;
        }
      }
    }
